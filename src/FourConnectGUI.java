...
public class FourConnectGUI extends JComponent implements MouseListener
{
	...
	tie = Toolkit.getDefaultToolkit().getImage("imgs/tie.png");
    ...
    else if (winner == IGameLogic.Winner.TIE)
	...
	if (winner == IGameLogic.Winner.NOT_FINISHED){
	    ...
	    if (playerTurn == 1) {
            if (player1==null){//human
                ...
                winner = player2.gameFinished(gameBoard);
                playerTurn = 2;
                }
            }
            else{//computer
                col = player1.decideNextMove(gameBoard);
                if (updateBoard(col,1)){
                    if (player2!=null) player2.insertCoin(col,1);
                    player1.insertCoin(col,1);
                    winner = player1.gameFinished(gameBoard);
                    playerTurn = 2;
                }
                else{
            ...
            if (player2==null){//human
                ...
                col = player2.decideNextMove(gameBoard);
                if (updateBoard(col,2)){
